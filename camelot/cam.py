...
class GameBoard(tk.Frame):
    def __init__(self, parent, named_1,
        ...
        self.plays = 0
        self.prev_turn = ''
        self.fakelist1 = self.location_list_1
        self.fakelist2 = self.location_list_2
    ...
    def move_image(self, event):
        ...
        if self.turn == "":
            ...
            for k, v in self.named_2.items():
                ...
                if v == move[0]:
                    ...
                    self.name_holder = k
                    print('nh', self.name_holder)
                    # self.remove_piece()

            self.named_2[self.name_holder] = move[1]
            y = move[1][0]
            x = move[1][1]
        ...
        if self.plays == 0 or self.plays == 1:
    ...
    def com_move(self):
        ...
        t = self.alphabeta_cutoff_search(self.location_list_1, self.location_list_2)
        tx = t[1]
        ty = t[0]
    ...
    def possible_moves(self, list1, list2, player):
        ...
        if player == 'b':
            for i in list2:
                cy = i[0]
                cx = i[1]
                p_finder = [((cy, cx), (cy + 1, cx)), ((cy, cx), (cy - 1, cx)), ((cy, cx), (cy + 1, cx + 1)),
                            ((cy, cx), (cy - 1, cx - 1)), ((cy, cx), (cy + 1, cx - 1)), ((cy, cx), (cy - 1, cx + 1)),
                            ((cy, cx), (cy, cx + 1)), ((cy, cx), (cy, cx - 1))]
                for i in p_finder:
                    if i[1] in self.background:
                        p_finder.remove(i)
                    for e in i[1]:
                        if e < 0:
                            p_finder.remove(i)
    ...
    def board_result(self, list1, list2, move, player):
        if player == 'w':
            print(move, 'move')
            print(list1, ' list1')
            try:
    ...
    def alphabeta_cutoff_search(self, state1, state2, d=6, cutoff_test=None, eval_fn=None):
        ...
        max_pru = 0
        state = state1 + state2
        self.fakelist1 = state1
        self.fakelist2 = state2
        def max_value(state1, state2, alpha, beta, depth):
            ...
            v = -10000
            for a in self.possible_moves(state1, state2, 'w'):
                node += 1

                # print(state)
                # print(self.game.result(state, a, 1))
                v = max(v, min_value(self.board_result(state1, state2, a, 'w')[0], self.board_result(state1, state2, a, 'w')[1], alpha, beta, depth + 1))
...
#            state2.remove(a[0])
        ...
        def min_value(state1, state2, alpha, beta, depth):
            ...
            if cutoff_test(state1, state2, depth, used_time):
                ...
                return eval_fn(state1, state2)
            v = 10000
            for a in self.possible_moves(state1, state2, 'b'):
                node += 1
               # state2.append(a[1])
                ...
                v = min(v, max_value(self.board_result(state1, state2, a, 'w')[0], self.board_result(state1, state2, a, 'w')[1], alpha, beta, depth + 1))
            #    state2.remove(a[1])
                # print(v)
                if v <= alpha:
                    min_pru += 1
...
#            state2.remove(a[0])
        ...
        for a in self.possible_moves(state1, state2, 'b'):
            ...
            v = min_value(self.board_result(state1, state2, a, 'b')[0], self.board_result(state1, state2, a, 'w')[1], best_score, beta, 1)
            print(v, 'tttvvv', best_score, ' bs')
            if v > best_score:
                best_score = v
                best_action = a
#        state2.remove(a[0])
